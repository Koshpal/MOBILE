...
class TagsFluxDeck(
        ...
        return list.filter {
            ...
            when {
                ...
                period.startsWith("date:") -> {
                    val targetDateStr = period.removePrefix("date:")
                    date.toString() == targetDateStr
                }
                else -> true
            }
        }
    ...
    private fun calculateInsight(transactions: Transactions, goals: List<Goal>, goal: Double): String {
        ...
        return when {
            ...
            spent > goal -> {
                val diff = spent - goal
                val whole = diff.toLong()
                val fraction = ((diff - whole) * 100).toInt()
                val fracStr = fraction.toString().padStart(2, '0')
                "You have exceeded your budget goal by ₹$whole.$fracStr."
            }
