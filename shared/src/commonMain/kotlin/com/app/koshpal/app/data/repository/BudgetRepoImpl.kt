...
class BudgetRepoImpl(
    ...
    override suspend fun archiveBudget(budgetId: String) {
        val history = BudgetHistoryEntity(budgetId = budgetId, archivedTimestamp = Clock.System.now().toEpochMilliseconds())
    ...
    override suspend fun getRemoteBudgets(token: String): Result<List<Budget>, NetworkError> {
        return withContext(defaultDispatcher) {
            val dtoResult = remoteDataSource.getBudgets(token)
            dtoResult.map { list -> list.map { it.toLocalBudget() } }
        }
    }

    override suspend fun deleteRemoteBudget(token: String, budgetId: String): Result<CommonResponse, NetworkError> {
        return withContext(defaultDispatcher) {
            val dtoResult = remoteDataSource.deleteBudget(token, budgetId)
            dtoResult.map { it.toCommonResponse() }
        }
    }
