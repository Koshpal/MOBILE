...
import com.app.koshpal.app.data.mapper.toEntity
import com.app.koshpal.app.data.mapper.toLocalTransactions
import com.app.koshpal.app.data.mapper.toTransactionsDto
import com.app.koshpal.app.data.mapper.toUiTransactions
...
class TransactionsRepoImpl(
    ...
    override suspend fun getTransactions(accessToken: String, page: Int, limit: Int): Result<Transactions, NetworkError> {
        if (userPreferences.isGuestUser.first()) return Result.Error(NetworkError.UNKNOWN, "Offline mode")
        return withContext(defaultDispatcher) {
            val dtoResult: Result<TransactionsDto, NetworkError> = remoteTransactionsDataSource.getTransactions(accessToken = accessToken, page = page, limit = limit)
