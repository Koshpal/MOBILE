...
class AuthRepoImpl(
    ...
    override suspend fun onRefreshToken(refreshToken: String, refreshTokenId: String): Result<User, NetworkError> {
        return withContext(defaultDispatcher) {
            val dtoResult = authDataSource.onRefreshToken(
                RefreshTokenRequestDto(refreshToken = refreshToken, refreshTokenId = refreshTokenId)
            )
            dtoResult.map { it.toUser() }
        }
    }
