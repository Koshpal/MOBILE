...
class CashCoordinator(
    ...
    private fun sync() {
        scope.launch {
            transactionQueries.getAllTransactionsInRange(0, Long.MAX_VALUE).collectLatest { fluxDeck.updateAllTransactions(it) }
        }
        scope.launch {
    ...
    fun deleteTransactions(transactions: Transactions) {
        scope.launch {
            transactionQueries.deleteLocalTransactions(transactions)
