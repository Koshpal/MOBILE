...
class GoalRepoImpl(
    ...
    override suspend fun getRemoteGoals(token: String): Result<List<Goal>, NetworkError> {
        return withContext(defaultDispatcher) {
            val dtoResult = remoteDataSource.getGoals(token)
            dtoResult.map { list -> list.map { it.toGoal() } }
        }
    }

    override suspend fun deleteRemoteGoal(token: String, goalId: String): Result<CommonResponse, NetworkError> {
        return withContext(defaultDispatcher) {
            val dtoResult = remoteDataSource.deleteGoal(token, goalId)
            dtoResult.map { it.toCommonResponse() }
        }
    }
}
