...
class TransactionsFluxDeck {
    ...
    private val _date = MutableStateFlow(Clock.System.now().toEpochMilliseconds())
    ...
    val filteredTransactions = combine(
        ...
        list.transactions.filter { transaction ->
            ...
            val normalizedBank = transaction.bank.replace("_", " ")
            val normalizedQuery = query.trim()
            val matchesQuery = partyName.contains(query, ignoreCase = true) ||
                    normalizedBank.contains(normalizedQuery, ignoreCase = true) ||
            ...
            val matchesNotes = if (withNotes) (transaction.description?.isNotBlank() == true) || !transaction.notes.isNullOrBlank() else true
    ...
    val partyActivityDates = activePartyTransactions.map { list ->
        list.map { txn ->
            val localDate = Instant.fromEpochMilliseconds(txn.transactionDate)
                .toLocalDateTime(TimeZone.currentSystemDefault()).date
            Instant.parse("${localDate}T00:00:00Z").toEpochMilliseconds()
        }.distinct().sorted()
    }
    ...
    val detailedFilteredTransactions = combine(activePartyTransactions, _selectedPartyDate) { list, date ->
        if (date == null) list
            ...
            val selectedLocalDate = Instant.fromEpochMilliseconds(date)
                .toLocalDateTime(TimeZone.currentSystemDefault()).date
            list.filter {
                val txnLocalDate = Instant.fromEpochMilliseconds(it.transactionDate)
                    .toLocalDateTime(TimeZone.currentSystemDefault()).date
    ...
    fun clearCreationDraft() {
        ...
        _bank.value = ""
        _mode.value = ""
        _date.value = Clock.System.now().toEpochMilliseconds()
