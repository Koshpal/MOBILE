...
class BudgetViewModel(
    ...
    fun addAllSelectedItem() {
        if (isItemClicked.value) {
            val allBudgets = fluxDeck.allBudgets.value + fluxDeck.archivedBudgets.value
    ...
    fun toggleIndividualFlaggedState(id: String) {
        coordinator.toggleIndividualFlaggedState(id, fluxDeck.allBudgets.value + fluxDeck.archivedBudgets.value)
    ...
    fun deleteSelectedBudgets() {
        val selectedIds = _selectedItemIds.value
        val all = fluxDeck.allBudgets.value + fluxDeck.archivedBudgets.value
        val toDelete = all.filter { selectedIds.contains(it.id) }
        toDelete.forEach { coordinator.deleteBudget(it) }
        resetEditingState()
    }
    ...
    fun toggleFlaggedState() {
        coordinator.toggleFlaggedState(_selectedItemIds.value.toList(), fluxDeck.allBudgets.value + fluxDeck.archivedBudgets.value)
        resetEditingState()
    }
