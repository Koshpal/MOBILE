...
fun String?.toBankDisplayName(): String {
    if (this.isNullOrBlank()) return "Unknown Bank"
    try {
        return Bank.valueOf(this).toDisplayName()
    } catch (_: Exception) {}
    try {
        val enumKey = this.trim().replace(" ", "_").uppercase()
        return Bank.valueOf(enumKey).toDisplayName()
    } catch (_: Exception) {}
