...
class SmsFilterImpl : SmsFilter {
    ...
    private fun isLikelyTransactional(sms: SmsMessage): Boolean {
        if (!hasBusinessSenderFormat(sms.sender)) {
            logDebug("KoshpalSMS: Rejected: Invalid sender format (${sms.sender})")
            return false
        }
        ...
        if (body.contains("http://") || body.contains("https://") || body.contains("www.") ||
            ...
            logDebug("KoshpalSMS: Rejected: Suspicious URL detected")
            return false
        }

        val matchedExclusion = EXCLUSION_KEYWORDS.find { it in body }
        if (matchedExclusion != null) {
            logDebug("KoshpalSMS: Rejected: Fraud/Non-transactional keyword '$matchedExclusion' found")
            return false
        }

        if (isSuspiciousActionRequest(body)) {
            logDebug("KoshpalSMS: Rejected: Potential phishing/fraud pattern detected")
            return false
        }

        val hasKeyword = TRANSACTION_KEYWORDS.any { it in body }
        if (!hasKeyword) {
            logDebug("KoshpalSMS: Rejected: No transaction keywords in body")
        } else {
            logDebug("KoshpalSMS: Accepted: Legitimate transaction identified")
        }
