...
class RemoteGoalDataSource(
    ...
    override suspend fun getGoals(accessToken: String): Result<List<GoalDto>, NetworkError> {
        ...
        return result.map { it.financialGoals ?: emptyList() }
    }
    ...
    override suspend fun deleteGoal(accessToken: String, goalId: String): Result<CommonResponseDto, NetworkError> {
        return safeCall<CommonResponseDto> {
            httpClient.delete(urlString = constructUrl("/employee/goals/$goalId")) {
                bearerAuth(accessToken)
            }
        }
    }
